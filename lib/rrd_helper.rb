...
connections.each_pair do |s,c|
    ...
    # generate graphs
    arg = "graph #{graph_dir}daily.jpg --start #{today} -t \"24 hour\" -w 897 " +
          "DEF:connections=#{file}:connections:AVERAGE LINE1:connections#217A2D:\"Connections\""
    `rrdtool #{arg}`
    arg = "graph #{graph_dir}weekly.jpg --start #{week}  -t \"7 day\" -w 897 " +
          " DEF:connections=#{file}:connections:AVERAGE LINE1:connections#217A2D:\"Connections\""
    `rrdtool #{arg}`
    arg = "graph #{graph_dir}monthly.jpg --start #{month}  -t \"30 day\" -w 897 " +
          " DEF:connections=#{file}:connections:AVERAGE LINE1:connections#217A2D:\"Connections\""
    `rrdtool #{arg}`
    arg = "graph #{graph_dir}yearly.jpg --start #{year}  -t \"365 day\" -w 897 " +
          " DEF:connections=#{file}:connections:AVERAGE LINE1:connections#217A2D:\"Connections\""
    `rrdtool #{arg}`
end
