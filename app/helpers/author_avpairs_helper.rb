module AuthorAvpairsHelper
    def author_avpair_description(author_avpair, include_mod=false)
        ...
        return("<ul style=\"list-style-type: none;\"><li>author-avpair-list #{author_avpair.name} 10</li><li><i> &nbsp&nbsp empty set</i></li></ul>") if (entries.length == 0)

        str = "<ul style=\"list-style-type: none;\">\n"
        entries.each do |e|
            str << "<li>author-avpair-list #{author_avpair.name} #{e.sequence}</li>\n"
            str << "<li> &nbsp&nbsp match access-list #{e.acl.name}</li>\n"if (e.acl_id)
