    ...
    def destroy
        respond_to do |format|
            ...
            if (@local_manager.slave?)
                ...
                @local_manager.log(:username => @session_user.username, :configuration_id => @configuration.id, :message => "Deleted User Group #{@user_group.name} from configuration #{@configuration.name}.")
    ...
    def update
        respond_to do |format|
            ...
            if (@local_manager.slave?)
                ...
                @local_manager.log(:username => @session_user.username, :user_group_id => @user_group.id, :configuration_id => @configuration.id, :message => "Updated User Group #{@user_group.name} within configuration #{@configuration.name}.")
                format.html { redirect_to user_groups_configuration_url(@configuration) }
                format.xml  { head :ok }
            else
                format.html { render :action => "edit" }
