    ...
    def destroy
        respond_to do |format|
            if (@local_manager.slave?)
                ...
                @local_manager.log(:username => @session_user.username, :configuration_id => @configuration.id, :message => "Deleted entry '#{@command_authorization_whitelist_entry.description}' of Configuration '#{@configuration.name}'.")
